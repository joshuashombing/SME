...
class OpenVINOInferencer(Inferencer):
    ...
    def load_model(self, path: str | Path | tuple[bytes, bytes]):
        ...
        # ie_core.set_property("CPU", {"CPU_THREADS_NUM": 3})

        compile_model = ie_core.compile_model(model=model, device_name="AUTO:CPU,GPU.0", config=self.config)
        # num_requests = compile_model.get_property(properties.optimal_number_of_infer_requests())
        # print(num_requests)
    ...
    def post_process(self, predictions: np.ndarray, metadata: dict | DictConfig | None = None) -> dict[str, Any]:
        ...
        if len(predictions.shape) == 1:
            ...
            cv2.circle(mask, (w // 2, h // 2), radius, (255, 255, 255), thickness=-1)
            anomaly_map[mask != 0] = anomaly_map.min()
