...
from anomalib.models.detection.utils import expand_bbox, resize_image
    ...
    def __init__(self):
        ...
        self.post_process_detections = {
            ...
            "kamera-samping": lambda img_: self.post_process_detection(
                img_, target_shape=(640, 320), rotate_image=False
            )
        }
        self.detector_path = "tools/yolov8.pt"
...
def detect(i, image):
    results = model.predict(image, i)

    for result in results:
        image = draw_bbox(image, result)
...
def main():
    ...
    # path = "C:\\Users\\maftuh.mashuri\\MVS\\Data\\MV-CU120-10GC (DA2704392)\\Video_20240422141229544.avi"
    # "C:\Users\maftuh.mashuri\MVS\Data\MV-CU120-10GC (DA2719677)\Video_20240422142017935.avi"
    # path = Path("C:\\Users\\maftuh.mashuri\\MVS\\Data\\MV-CU120-10GC (DA2719677)\\Video_20240422143311575.avi")
    # predict_video(model, path, "kamera-atas")
...
def main2():
    predictor = SpringMetalInspector()
    predictor._build_model()

    data_paths = get_data_path_generator()
    data_paths["atas"] = list(data_paths["atas"][0])
    data_paths["samping"] = list(data_paths["samping"][0])
    print(data_paths)
