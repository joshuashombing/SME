...
if __name__ == "__main__":
    ...
    def display_frame():
        ...
        if frame is not None:
            update_image_on_widget(frame)

    def update_image_on_widget(frame):
        # Konversi frame dari format NumPy ke QImage
        ...
        # Displaying the processed image in widgetImagePredictTop
        widget_size = ui.widgetImagePreprocessingTop.size()
        ui.widgetImagePreprocessingTop.setPixmap(pixmap.scaled(widget_size, Qt.KeepAspectRatio))

    def display_frames_continuously():
        while isGrabbing:
            display_frame()
            time.sleep(0.01)
    ...
    def start_grabbing():
        ...
        Returns:
            None
        """
        if not isGrabbing:  # Check if the camera is not already grabbing
            win_id = ui.widgetDisplay.winId()
            ...
            if ret != 0:
                ...
                thread_display = threading.Thread(target=display_frames_continuously)
