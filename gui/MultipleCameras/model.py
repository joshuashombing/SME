...
class SpringMetalInspector:
    ...
    def predict(self, image: np.ndarray, model_name):
        ...
        if self.fps.elapsed() >= 1:
            self.collected_fps.append(self.fps.fps())
            print(f"Speed {model_name}", self.fps.fps(), "FPS")
            self.fps.start()

        if len(self.collected_fps) >= 10:
            print(f"Mean Speed per 10s {model_name} {np.mean(self.collected_fps):.4f} FPS")
            self.collected_fps = []
